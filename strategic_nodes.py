...
class NSEOptionChain:
    ...
    # Rotate UA strings — each warm-up attempt uses a different one
    _UA_LIST = [
    ...
    def _make_headers(self, ua=None):
        return {
            ...
            "user-agent":         ua or self._next_ua(),
    ...
    def _make_session(self):
        """
        Warm up NSE session with 4-step cookie harvesting + API probe.
        Retries 3 times with different UA strings if probe fails.
        Steps: homepage → market-data → derivatives → option-chain → API probe
        """
        ...
        for attempt in range(1, 4):
            ...
            try:
                # Step 1 — homepage (plants initial nseappid + nsit cookies)
                r1 = session.get("https://www.nseindia.com/",
                                 headers=nav_headers, impersonate="chrome124", timeout=20)
                print(f"    Homepage      : {r1.status_code} | cookies: {len(session.cookies)}")
                time.sleep(2.0)

                # Step 2 — market-data page (required by NSE CDN)
                ...
                # Step 3 — derivatives watch page (primes additional API cookies)
                r3 = session.get("https://www.nseindia.com/market-data/equity-derivatives-watch",
                                 headers=nav_headers, impersonate="chrome124", timeout=20)
                print(f"    Derivatives   : {r3.status_code} | cookies: {len(session.cookies)}")
                time.sleep(1.5)

                # Step 4 — option-chain page (final cookie before API)
                r4 = session.get("https://www.nseindia.com/option-chain",
                                 headers=nav_headers, impersonate="chrome124", timeout=20)
                print(f"    Option-chain  : {r4.status_code} | cookies: {len(session.cookies)}")
                time.sleep(2.5)

                # Step 5 — probe the API endpoint to confirm session is live
                probe = session.get(
                ...
                if probe.status_code == 200:
                    print("    ✓ Session confirmed live")
                    return session, headers
                print(f"    ✗ Probe failed ({probe.status_code}) — rotating UA and retrying...")
                time.sleep(5)
            except Exception as e:
                print(f"    WARNING attempt {attempt}: {e}")
                time.sleep(5)

        print("  WARNING: Could not confirm live session — proceeding anyway (may fail)")
    ...
    def _fetch_for_expiry(self, session, headers, expiry):
        ...
        for attempt in range(1, 4):
            try:
                ...
                if resp.status_code != 200:
                    print(f"    HTTP {resp.status_code} on attempt {attempt} | url={api_url[:80]}")
                ...
                try:
                    ...
                    print(f"    JSON parse error: {je} | body: {resp.text[:200]}")
                    time.sleep(2)
                    continue
                data = json_data.get("records", {}).get("data", [])
                if not data:
                    print(f"    Empty records in JSON — keys: {list(json_data.keys())}")
                    return None
...
def main():
    ...
    if not all_expiry_data:
        print("\nERROR: No data fetched from NSE.")
        print("  Possible causes:")
        print("  1. NSE is blocking this runner IP (most common)")
        print("  2. Market is closed and NSE API returned empty data")
        print("  3. curl_cffi version mismatch — try: pip install -U curl_cffi")
        print("  Exiting with code 1 so GitHub Actions marks this run as FAILED.")
