...
.cv-iv{{font-size:12px;color:#d8eeff;font-family:'JetBrains Mono',monospace;text-align:right;}}
.cv-oi{{font-size:12px;font-family:'JetBrains Mono',monospace;color:#d8eeff;text-align:right;}}
...
.strat-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:12px;}}
.strat-card{{background:var(--bg3);border:1px solid var(--border);border-left:3px solid var(--cc,var(--cyan));border-radius:11px;overflow:hidden;transition:all .25s;cursor:pointer;padding:0;}}
.strat-card:hover{{transform:translateY(-3px);box-shadow:0 8px 28px rgba(0,0,0,.5);border-color:var(--cc,var(--cyan));}}
.sc-compact-top{{padding:14px 16px 10px;display:flex;align-items:flex-start;justify-content:space-between;gap:8px;}}
.sc-compact-name{{font-size:17px;font-weight:800;color:#fff;margin-bottom:3px;}}
.sc-compact-sub{{font-size:12px;color:#c8dff0;font-family:'DM Mono',monospace;}}
.sc-compact-pop{{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:8px 12px;border-radius:9px;min-width:58px;text-align:center;}}
.sc-compact-pop .pop-num{{font-size:20px;font-weight:800;font-family:'DM Mono',monospace;line-height:1;}}
.sc-compact-pop .pop-txt{{font-size:11px;font-weight:700;margin-top:2px;}}
.sc-compact-stats{{display:grid;grid-template-columns:1fr 1fr;gap:1px;background:var(--border);border-top:1px solid var(--border);border-bottom:1px solid var(--border);}}
.sc-cstat{{background:var(--bg2);padding:10px 14px;}}
.sc-cstat-lbl{{font-size:11px;color:#9dbdd8;font-family:'DM Mono',monospace;text-transform:uppercase;letter-spacing:.7px;margin-bottom:3px;}}
.sc-cstat-val{{font-size:15px;font-weight:700;font-family:'DM Mono',monospace;}}
.sc-compact-legs{{padding:10px 16px 12px;display:flex;flex-wrap:wrap;gap:6px;}}
.sc-cleg{{display:flex;align-items:center;gap:5px;padding:5px 10px;border-radius:6px;font-family:'DM Mono',monospace;font-size:12px;font-weight:700;}}
.sc-cleg.buy{{background:rgba(0,200,150,.12);border:1px solid rgba(0,200,150,.3);color:#00c896;}}
.sc-cleg.sell{{background:rgba(255,107,107,.12);border:1px solid rgba(255,107,107,.3);color:#ff8080;}}
.sc-compact-footer{{padding:8px 16px;border-top:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;}}
.sc-open-btn{{font-size:12px;font-weight:700;color:#00d4ff;font-family:'DM Mono',monospace;letter-spacing:.5px;display:flex;align-items:center;gap:5px;}}
.sc-fit-mini{{font-size:12px;font-weight:700;font-family:'DM Mono',monospace;}}

/* ── STRATEGY MODAL ── */
.strat-modal-overlay{{display:none;position:fixed;inset:0;background:rgba(4,8,16,.88);z-index:10000;backdrop-filter:blur(6px);overflow-y:auto;padding:20px;}}
.strat-modal-overlay.open{{display:flex;align-items:flex-start;justify-content:center;}}
.strat-modal{{background:#0d1420;border:1px solid #1e3048;border-radius:16px;width:100%;max-width:820px;margin:auto;overflow:hidden;box-shadow:0 24px 80px rgba(0,0,0,.8);animation:modalIn .25s ease;}}
@keyframes modalIn{{from{{opacity:0;transform:translateY(20px);}}to{{opacity:1;transform:translateY(0);}}}}
.modal-hdr{{padding:20px 24px 16px;background:linear-gradient(135deg,#0d1a2b,#111826);border-bottom:1px solid #1e3048;display:flex;align-items:flex-start;justify-content:space-between;gap:12px;}}
.modal-title{{font-size:22px;font-weight:800;color:#fff;margin-bottom:4px;}}
.modal-sub{{font-size:13px;color:#c8dff0;font-family:'DM Mono',monospace;}}
.modal-close{{width:36px;height:36px;border-radius:8px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);color:#fff;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:background .15s;}}
.modal-close:hover{{background:rgba(255,107,107,.2);border-color:#ff6b6b;color:#ff6b6b;}}
.modal-body{{padding:20px 24px;}}
.modal-kpis{{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:20px;}}
.modal-kpi{{background:#111826;border:1px solid #1e3048;border-radius:10px;padding:14px 16px;text-align:center;}}
.modal-kpi-lbl{{font-size:11px;color:#9dbdd8;font-family:'DM Mono',monospace;text-transform:uppercase;letter-spacing:.7px;margin-bottom:6px;}}
.modal-kpi-val{{font-size:20px;font-weight:800;font-family:'DM Mono',monospace;}}
.modal-section-title{{font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:#9dbdd8;font-family:'DM Mono',monospace;margin-bottom:12px;display:flex;align-items:center;gap:7px;}}
.modal-legs{{display:flex;flex-direction:column;gap:8px;margin-bottom:20px;}}
.modal-leg{{display:flex;align-items:flex-start;gap:12px;padding:14px 16px;border-radius:10px;}}
.modal-leg.buy{{background:rgba(0,200,150,.08);border:1px solid rgba(0,200,150,.25);}}
.modal-leg.sell{{background:rgba(255,107,107,.08);border:1px solid rgba(255,107,107,.25);}}
.modal-leg-badge{{padding:5px 12px;border-radius:6px;font-size:13px;font-weight:800;font-family:'DM Mono',monospace;flex-shrink:0;}}
.modal-leg.buy .modal-leg-badge{{background:rgba(0,200,150,.2);color:#00c896;}}
.modal-leg.sell .modal-leg-badge{{background:rgba(255,107,107,.2);color:#ff8080;}}
.modal-leg-info{{flex:1;}}
.modal-leg-main{{font-size:16px;font-weight:700;font-family:'DM Mono',monospace;color:#fff;margin-bottom:4px;}}
.modal-leg-why{{font-size:13px;color:#c8dff0;font-family:'DM Mono',monospace;line-height:1.5;}}
.modal-bes{{display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap;}}
.modal-be-pill{{padding:10px 20px;border-radius:9px;background:rgba(255,209,102,.08);border:1px solid rgba(255,209,102,.28);text-align:center;}}
.modal-be-lbl{{font-size:11px;color:#9dbdd8;font-family:'DM Mono',monospace;text-transform:uppercase;margin-bottom:4px;}}
.modal-be-val{{font-size:18px;font-weight:800;font-family:'DM Mono',monospace;color:#ffd166;}}
.modal-be-accuracy{{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px;}}
.modal-acc-card{{background:#111826;border:1px solid #1e3048;border-radius:10px;padding:14px 16px;}}
.modal-acc-side{{font-size:12px;font-weight:700;color:#9dbdd8;font-family:'DM Mono',monospace;text-transform:uppercase;margin-bottom:10px;}}
.modal-acc-row{{display:flex;align-items:center;justify-content:space-between;gap:8px;}}
.modal-acc-col{{text-align:center;}}
.modal-acc-lbl{{font-size:11px;color:#6a90b8;font-family:'DM Mono',monospace;margin-bottom:4px;}}
.modal-acc-val{{font-size:17px;font-weight:800;font-family:'DM Mono',monospace;}}
.modal-acc-arrow{{font-size:20px;color:#d8eeff;}}
.modal-fit-bar{{margin-bottom:20px;}}
.modal-fit-hdr{{display:flex;justify-content:space-between;font-size:13px;font-weight:700;font-family:'DM Mono',monospace;color:#d8eeff;margin-bottom:8px;}}
.modal-fit-track{{height:8px;background:#1e3048;border-radius:4px;overflow:hidden;}}
.modal-fit-fill{{height:100%;border-radius:4px;transition:width 1s ease;}}
.modal-insight{{padding:14px 16px;background:rgba(255,209,102,.06);border:1px solid rgba(255,209,102,.18);border-radius:10px;font-size:13px;font-family:'DM Mono',monospace;color:#ffd166;line-height:1.6;margin-bottom:20px;}}
.modal-sim-wrap{{border:1px solid #1e3048;border-radius:10px;overflow:hidden;}}
.modal-warn{{margin-bottom:12px;padding:10px 16px;border-radius:8px;font-size:13px;font-weight:700;font-family:'DM Mono',monospace;}}
.modal-warn.poor{{background:rgba(255,107,107,.12);border:1px solid rgba(255,107,107,.3);color:#ff8080;}}
.modal-warn.low{{background:rgba(255,209,102,.10);border:1px solid rgba(255,209,102,.3);color:#ffd166;}}
...
.sc-name{{font-size:17px;font-weight:800;color:#ffffff;}}
...
// ── Render Strategy Cards (handles both SR & BE modes) ───────
function renderStrategies() {{
  ...
  document.getElementById("stratGrid").innerHTML = sorted.map((s,i)=>{{
    ...
    const poorValueBadge    = (s.isBEMode && s.poorValue)
      ? `<div style="margin:0 0 2px;padding:6px 14px;background:rgba(255,107,107,.15);border-bottom:1px solid rgba(255,107,107,.40);font-size:12px;font-weight:800;color:#ff8080;font-family:'DM Mono',monospace;letter-spacing:.8px;">⚠️ POOR VALUE — Yield-to-Risk ${{s.yieldToRisk.toFixed(2)}}x (below 1:5 threshold). Consider a wider spread.</div>`
      : "";
    const lowLiquidityBadge = (s.isBEMode && s.lowLiquidity)
      ? `<div style="margin:0 0 2px;padding:6px 14px;background:rgba(255,209,102,.14);border-bottom:1px solid rgba(255,209,102,.40);font-size:12px;font-weight:800;color:#ffd166;font-family:'DM Mono',monospace;letter-spacing:.8px;">⚠️ LOW LIQUIDITY — One or more sell legs have zero OI. Verify fills before trading.</div>`
      : "";
            ...
            return `<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:7px;padding:7px 10px;">
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;gap:6px;">
                <div style="text-align:center;">
                  <div style="font-size:11px;font-weight:700;color:#c8dff0;font-family:'DM Mono',monospace;">YOUR INPUT</div>
                ...
                <div style="text-align:center;">
                  <div style="font-size:11px;font-weight:700;color:#c8dff0;font-family:'DM Mono',monospace;">ACTUAL BE</div>
                  <div style="font-size:15px;font-weight:800;font-family:'DM Mono',monospace;color:${{col}};">₹${{a.actual.toLocaleString("en-IN")}}</div>
                </div>
                <div style="text-align:center;">
                  <div style="font-size:11px;font-weight:700;color:#c8dff0;font-family:'DM Mono',monospace;">DIFF</div>
    ...
    // ── BE mode: detailed leg rows with why explanations ──
    ...
    // ── Compact legs for card preview ──────────────────────────────
    const compactLegs = s.legs.map(l=>`
      <div class="sc-cleg ${{l.action}}">
        <span>${{l.action==="buy"?"▲":"▼"}}</span>
        <span>${{l.opt_type||l.type}} ₹${{l.strike.toLocaleString("en-IN")}}</span>
        <span style="opacity:.7;">@₹${{l.premium}}</span>
      </div>`).join("");

    const fitColor = s.fit>=80?"var(--green)":s.fit>=60?"var(--gold)":"var(--red)";
    const fitMini = s.isBEMode
      ? `<span class="sc-fit-mini" style="color:${{fitColor}};">FIT ${{s.fit}}%</span>`
      : `<span class="sc-fit-mini" style="color:var(--text3);">—</span>`;

    return `<div class="strat-card" style="--cc:${{cc}};animation-delay:${{i*0.05}}s"
      onclick="openStratModal(${{JSON.stringify(s).replace(/`/g,'\\`')}})">
      <div class="sc-compact-top">
        <div style="flex:1;min-width:0;">
          <div class="sc-compact-name">${{emojis[s.biasTag]||"📊"}} ${{s.name}}</div>
          <div class="sc-compact-sub">${{s.biasTag.toUpperCase()}} · ${{s.isDebit?"DEBIT":"CREDIT"}} · DTE:${{ALL_DATA[currentExpiry]?.dte||"—"}}</div>
        </div>
        <div class="sc-compact-pop" style="background:${{popBg}};border:1px solid ${{popCol}}44;">
          <div class="pop-num" style="color:${{popCol}};">${{s.pop}}%</div>
          <div class="pop-txt" style="color:${{popCol}};">PoP</div>
        </div>
      </div>
      <div class="sc-compact-stats">
        <div class="sc-cstat">
          <div class="sc-cstat-lbl">Max Profit</div>
          <div class="sc-cstat-val" style="color:var(--green);">₹${{s.maxProfit.toLocaleString("en-IN")}}</div>
        </div>
        <div class="sc-cstat">
          <div class="sc-cstat-lbl">Max Loss</div>
          <div class="sc-cstat-val" style="color:var(--red);">₹${{s.maxLoss.toLocaleString("en-IN")}}</div>
        </div>
        <div class="sc-cstat">
          <div class="sc-cstat-lbl">RR Ratio</div>
          <div class="sc-cstat-val" style="color:var(--gold);">1:${{rrDisp}}</div>
        </div>
        <div class="sc-cstat">
          <div class="sc-cstat-lbl">Breakevens</div>
          <div class="sc-cstat-val" style="font-size:12px;color:#d8eeff;">${{beStr}}</div>
        </div>
      </div>
      <div class="sc-compact-legs">${{compactLegs}}</div>
      <div class="sc-compact-footer">
        ${{fitMini}}
        <div class="sc-open-btn">VIEW DETAILS →</div>
      </div>
    </div>`;
  }}).join("");
}}
...
function openStratModal(s) {{
  ...
  const dte = d?.dte || "—";
  const atm = d?.atm_strike;
  const cc = s.biasTag==="bullish"?"var(--green)":s.biasTag==="bearish"?"var(--red)":"var(--cyan)";
  const popBg = s.biasTag==="bullish"?"rgba(0,200,150,.12)":s.biasTag==="bearish"?"rgba(255,107,107,.12)":"rgba(91,163,255,.12)";
  const popCol = s.biasTag==="bullish"?"#00c896":s.biasTag==="bearish"?"#ff6b6b":"#5ba3ff";
  const emojis={{bullish:"🐂",bearish:"🐻",neutral:"⚖️"}};
  const rrDisp = s.rrRatio ? s.rrRatio.toFixed(2) : "—";
  const bes = s.breakevens||[];
  const beStr = bes.length ? bes.map(b=>"₹"+b.toLocaleString("en-IN")).join(" / ") : "—";
  const fitColor = s.fit>=80?"#00c896":s.fit>=60?"#ffd166":"#ff6b6b";
  const netDisp = s.netPremium>=0
    ? `<span style="color:#00c896;">+₹${{(s.netPremium*65).toLocaleString("en-IN")}} cr</span>`
    : `<span style="color:#ff6b6b;">−₹${{Math.abs(s.netPremium*65).toLocaleString("en-IN")}} db</span>`;

  // Warnings
  let warnings = "";
  if(s.poorValue) warnings += `<div class="modal-warn poor">⚠️ POOR VALUE — Yield-to-Risk ${{s.yieldToRisk?.toFixed(2)}}x (below 1:5 threshold). Consider a wider spread.</div>`;
  if(s.lowLiquidity) warnings += `<div class="modal-warn low">⚠️ LOW LIQUIDITY — One or more sell legs have zero OI. Verify fills before trading.</div>`;

  // KPIs
  const kpis = `<div class="modal-kpis">
    <div class="modal-kpi">
      <div class="modal-kpi-lbl">Probability of Profit</div>
      <div class="modal-kpi-val" style="color:${{popCol}};">${{s.pop}}%</div>
    </div>
    <div class="modal-kpi">
      <div class="modal-kpi-lbl">Max Profit</div>
      <div class="modal-kpi-val" style="color:#00c896;">₹${{s.maxProfit.toLocaleString("en-IN")}}</div>
    </div>
    <div class="modal-kpi">
      <div class="modal-kpi-lbl">Max Loss</div>
      <div class="modal-kpi-val" style="color:#ff6b6b;">₹${{s.maxLoss.toLocaleString("en-IN")}}</div>
    </div>
    <div class="modal-kpi">
      <div class="modal-kpi-lbl">RR Ratio</div>
      <div class="modal-kpi-val" style="color:#ffd166;">1:${{rrDisp}}</div>
    </div>
    <div class="modal-kpi">
      <div class="modal-kpi-lbl">Breakevens</div>
      <div class="modal-kpi-val" style="font-size:14px;color:#ffd166;">${{beStr}}</div>
    </div>
    <div class="modal-kpi">
      <div class="modal-kpi-lbl">Net Credit/Debit</div>
      <div class="modal-kpi-val" style="font-size:16px;">${{netDisp}}</div>
    </div>
    <div class="modal-kpi">
      <div class="modal-kpi-lbl">Est. Margin</div>
      <div class="modal-kpi-val" style="font-size:15px;color:#8aa0ff;">₹${{s.margin.toLocaleString("en-IN")}}</div>
    </div>
    <div class="modal-kpi">
      <div class="modal-kpi-lbl">Score</div>
      <div class="modal-kpi-val" style="color:#ffd166;">${{s.score}}</div>
    </div>
  </div>`;

  // Legs
  const legsHtml = `<div class="modal-section-title">📋 LEGS TO TRADE</div>
    <div class="modal-legs">
      ${{s.legs.map(l=>`
        <div class="modal-leg ${{l.action}}">
          <div class="modal-leg-badge">${{l.action.toUpperCase()}}</div>
          <div class="modal-leg-info">
            <div class="modal-leg-main">
              <span style="color:${{(l.opt_type||l.type)==="CE"?"#00c896":"#ff6b6b"}};">${{l.opt_type||l.type}}</span>
              &nbsp;₹${{l.strike.toLocaleString("en-IN")}}
              <span style="color:#9dbdd8;font-size:14px;"> @ ₹${{l.premium}}</span>
            </div>
            ${{l.why ? `<div class="modal-leg-why">↳ ${{l.why}}</div>` : ""}}
          </div>
        </div>`).join("")}}
    </div>`;
  ...
  // BE accuracy
  let beAccHtml = "";
  if(s.isBEMode && s.beAccuracy && s.beAccuracy.length) {{
    beAccHtml = `<div class="modal-section-title">🎯 YOUR INPUT vs ACTUAL BREAKEVENS</div>
      <div class="modal-be-accuracy">
        ${{s.beAccuracy.map(a=>{{
          const sign=a.diff>=0?"+":"";
          const col=Math.abs(a.diff)<=30?"#00c896":Math.abs(a.diff)<=80?"#ffd166":"#ff6b6b";
          const icon=Math.abs(a.diff)<=30?"✅":Math.abs(a.diff)<=80?"⚠️":"❌";
          return `<div class="modal-acc-card">
            <div class="modal-acc-side">${{icon}} ${{a.side.toUpperCase()}} BE</div>
            <div class="modal-acc-row">
              <div class="modal-acc-col">
                <div class="modal-acc-lbl">YOUR INPUT</div>
                <div class="modal-acc-val" style="color:#ffd166;">₹${{a.target.toLocaleString("en-IN")}}</div>
              </div>
              <div class="modal-acc-arrow">→</div>
              <div class="modal-acc-col">
                <div class="modal-acc-lbl">ACTUAL BE</div>
                <div class="modal-acc-val" style="color:${{col}};">₹${{a.actual.toLocaleString("en-IN")}}</div>
              </div>
              <div class="modal-acc-col">
                <div class="modal-acc-lbl">DIFF</div>
                <div class="modal-acc-val" style="color:${{col}};">${{sign}}${{a.diff}}</div>
              </div>
            </div>
          </div>`;
        }}).join("")}}
  ...
  let fitBarHtml = "";
  if(s.isBEMode) {{
    fitBarHtml = `<div class="modal-fit-bar">
      <div class="modal-fit-hdr"><span>Breakeven Fit Score</span><span style="color:${{fitColor}};">${{s.fit}}%</span></div>
      <div class="modal-fit-track"><div class="modal-fit-fill" style="width:${{s.fit}}%;background:${{fitColor}};box-shadow:0 0 8px ${{fitColor}}55;"></div></div>
    </div>`;
  }}

  // Insight
  const insightHtml = s.beInsight
    ? `<div class="modal-insight">💡 ${{s.beInsight}}</div>`
    : "";

  // Simulator
  const uid = "modal_" + s.name.replace(/\s+/g,"_");
  const simHtml = `<div class="modal-section-title">📊 INTRADAY P&L SIMULATOR</div>
    <div class="modal-sim-wrap">${{buildIntradaySim(s, uid)}}</div>`;

  // Assemble
  document.getElementById("modalTitle").textContent = ({{bullish:"🐂",bearish:"🐻",neutral:"⚖️"}}[s.biasTag]||"📊") + " " + s.name;
  document.getElementById("modalSub").textContent   = s.biasTag.toUpperCase() + " · " + (s.isDebit?"DEBIT":"CREDIT") + " SPREAD · DTE:" + dte + "  |  ATM ₹" + (atm||"—");
  document.getElementById("modalBody").innerHTML    = warnings + kpis + legsHtml + beAccHtml + fitBarHtml + insightHtml + simHtml;
  document.getElementById("stratModal").classList.add("open");
  document.body.style.overflow = "hidden";
  // Init sim tab
  const simSc = document.getElementById("sim-sc-"+uid);
  if(simSc) simSc.style.display = "block";
}}
function closeStratModal(e) {{
  if(e.target === document.getElementById("stratModal")) closeStratModalBtn();
}}
function closeStratModalBtn() {{
  document.getElementById("stratModal").classList.remove("open");
  document.body.style.overflow = "";
}}
document.addEventListener("keydown", e=>{{ if(e.key==="Escape") closeStratModalBtn(); }});

</script>

<!-- STRATEGY DETAIL MODAL -->
<div class="strat-modal-overlay" id="stratModal" onclick="closeStratModal(event)">
  <div class="strat-modal" id="stratModalBox">
    <div class="modal-hdr">
      <div>
        <div class="modal-title" id="modalTitle">—</div>
        <div class="modal-sub" id="modalSub">—</div>
      </div>
      <button class="modal-close" onclick="closeStratModalBtn()">✕</button>
    </div>
